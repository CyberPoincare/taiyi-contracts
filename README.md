...
太乙系统底层暴露给SGS脚本的接口（即 C++ 层暴露给 Lua 合约的 API），包括了类型定义、全局方法、合约处理器 (`contract_helper`) 以及 NFA 处理器 (`nfa_helper`)。这些API的帮助文件详见[智能游戏脚本（合约）API 说明文档](https://github.com/hongzhongx/taiyi/blob/main/doc/sgs_api.md)。
