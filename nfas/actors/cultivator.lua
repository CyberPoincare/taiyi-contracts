...
function try_xiuzhen()
    ...
    -- 主动更新修真进度
    local update = import_contract("contract.cmds.std.cultivation").update
    update()
end
...
function on_actor_talking(someone, something)
    local base_on_actor_talking = import_contract('contract.actor.normal').on_actor_talking
    base_on_actor_talking(someone, something)
end

-- 听话回调函数
function on_actor_listening(from_who, something)
    local base_on_actor_listening = import_contract('contract.actor.normal').on_actor_listening
